...
drop column column6,column7,column8,column9,column10,column11,column12,column13,column14,column15,column16,column17,column18,column19,column20,column21,column22,column23,column24,column25,column26;
...
select distinct state from Dataset where lower(state) like 'a%' or lower(state) like 'b%'

select distinct state from Dataset where lower(state) like 'a%' and lower(state) like '%m'
...
from Dataset a
inner join 
Dataset1 b on a.district=b.district ) c) d
group by d.state;
...
from Dataset a 
inner join 
Dataset1 b on a.district=b.district) d) c
group by c.state
...
from Dataset a 
inner join 
Dataset1 b on a.district=b.district) d) e
group by e.state)m
